...
music <- read.csv("../input/top-spotify-tracks-of-2018/top2018.csv", header=TRUE)
...
set.seed(88)
wordcloud(names$artists, freq=names$count, min.freq=1,max.words=100,random.order=FALSE,rot.per=0.35,colors=brewer.pal(8, "Dark2"), main="Artists")
...
\
I played some fraction of 5 songs, hope you can enjoy you can find a connection between the music
and the data!

<iframe width="560" height="315" src="https://www.youtube.com/embed/fSzN9kd6urY" frameborder="0" allowfullscreen></iframe>
...
In my point of view, Post Malones songs have relatively connected melody lines compare to other rappers. His music flows more and the lyrics are not too dense. His voice 
...
\
*P.S Galigali likes Drake!吴亦凡喜欢Drake.*
...
## Overall Popular songs trend EDA {.tabset .tabset-fade .tabset-pills}
...
```{r}
c1<- ggplot(music, aes(x=danceability))+ geom_histogram(binwidth=1, colour="white", fill="darkseagreen2", alpha=0.8)+
  geom_density(eval(bquote(aes(y=..count..*1))),colour="darkgreen", fill="darkgreen", alpha=0.3) +labs(title="Danceability Distribution") + theme_minimal(base_size = 8)
c1
```

### Overall Energy Level Distribution

```{r}
c2<- ggplot(music, aes(x=energy))+ geom_histogram(binwidth=1, colour="white", fill="mediumpurple2", alpha=0.8)+
  geom_density(eval(bquote(aes(y=..count..*1))),colour="mediumorchid1", fill="mediumorchid1", alpha=0.3) +labs(title="Energy Distribution") + theme_minimal(base_size = 8)
c2
```

### Overall Speechiness Level Distribution

```{r}
c3<- ggplot(music, aes(x=speechiness))+ geom_histogram(binwidth=1, colour="white", fill="lightpink1", alpha=0.8)+
  geom_density(eval(bquote(aes(y=..count..*1))),colour="mistyrose2", fill="mistyrose2", alpha=0.3) +labs(title="Speechiness Distribution") + theme_minimal(base_size = 8)
c3
```

### Overall Acousticness Level Distribution

```{r}
c4<- ggplot(music, aes(x=acousticness))+ geom_histogram(binwidth=1, colour="white", fill="lightskyblue2", alpha=0.8)+
  geom_density(eval(bquote(aes(y=..count..*1))),colour="lightsteelblue", fill="lightsteelblue", alpha=0.3) +labs(title="Acousticness Distribution") + theme_minimal(base_size = 8)
c4
```

### Overall Liveness Level Distribution

```{r}
c5<- ggplot(music, aes(x=liveness))+ geom_histogram(binwidth=1, colour="white", fill="lightsalmon", alpha=0.8)+
  geom_density(eval(bquote(aes(y=..count..*1))),colour="lightcoral", fill="lightcoral", alpha=0.3) +labs(title="Liveness Distribution") + theme_minimal(base_size = 8)
c5
```

### Overall Valence Level Distribution

```{r}
c6<- ggplot(music, aes(x=valence))+ geom_histogram(binwidth=1, colour="white", fill="lightgoldenrod", alpha=0.8)+
  geom_density(eval(bquote(aes(y=..count..*1))),colour="moccasin", fill="moccasin", alpha=0.3) +labs(title="Valence Distribution") + theme_minimal(base_size = 8)
c6
```
...
## Heatmap + Scatterplot for Valence respect to Key and Tempo
...
## Heatmap + Scatterplot for Danceability respect to Key and Tempo
...
## Heatmap + Scatterplot for Energy respect to Key and Tempo
...
## Heatmap + Scatterplot for  Speechiness respect to Key and Tempo
...
# 🖼 Mosaic Plot Keys vs Tempo

Now we want to inverstigate if the key has relationship with tempo . From the mosaic graph, there is no evidence to show Minor Key or Major Key has association with the tempo.
...
# 👵🏻👴🏻 Happy Capriccio Concert + LingLing Wanna Be 

I am not only do not have perfect pitch, but also dont have relative pitch, horrible at rhythm, 
...
\
Looking back, it doesnt matter what genre of music we listen to, or what instruments we play.
\
Music did tie us together, as a community, we grow stronger.
\
I really like what Twoset Violin Fan Concert Organizer Lita (莉塔) wrote:

> Sometimes we feel we can do better after a recording,\
  Sometimes we feel we can never produce that sound again.\
  Sometimes we feel dragging the standmates,\
  Sometimes we have to slow down to wait for other players.\
  Sometimes we fought over an upbow stroke, other times we laughed about speeding up.\
  Music soothe the pain and tender the time,\
  As listeners and players, we were also cured and enriched by music!
...
[3] Music Sheet/ Back sound track
    https://www.youtube.com/watch?v=fADGYKDVrWk
    https://musescore.com/user/16006641/scores/4905239
    https://mp.weixin.qq.com/s/BRrk5Cn5SyriMtiM7cXt2Q
    https://mp.weixin.qq.com/s/1G9kz_dpORWXJQrBXnKo2Q
    https://www.youtube.com/watch?v=eAQqp4Mr1KQ
